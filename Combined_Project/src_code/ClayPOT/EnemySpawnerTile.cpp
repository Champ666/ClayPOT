...
EnemySpawnerTile::EnemySpawnerTile(int grid_x, int grid_y, float gridSizeF,
	...
	int enemy_type, int enemy_amount, int32_t enemy_time_to_spawn, float enemy_max_distance)
...
const std::string EnemySpawnerTile::getAsString() const
{
	...
	ss << this->type << " " << this->shape.getTextureRect().position.x << " " << this->shape.getTextureRect().position.y << " "
